	...
	<main class="mx-auto max-w-7xl px-6 py-8">
		...
		{:else if error}
			<div class="py-16 text-center">
				<div
					class="mb-4 inline-block rounded-lg px-4 py-3"
					style="background:#3a1a1a; border:0.5px solid #7a3020; color:#e8a0a0; font-size:13px;"
				>
					{error}
				</div>
				<div>
					<button
						...
						style="background:#4a5520; color:#c8d870; border:none; cursor:pointer;">Retry</button
					>
				</div>
			</div>
				...
				{#each filteredVideos as video, index (`${video._id || video.id || 'v'}-${index}`)}
					...
					>
						...
						<div style="padding:12px 14px;">
							<h3
								style="font-size:14px; font-weight:500; color:#c8d870; margin-bottom:6px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"
							>
								{video.name}
							</h3>
							<div style="font-size:12px; color:#5a6828; line-height:1.6;">
								{#if video.duration}<div>
										Duration: <span style="color:#a0b040;">{formatDuration(video.duration)}</span>
									</div>{/if}
								{#if video.size}<div>
										Size: <span style="color:#a0b040;">{formatFileSize(video.size)}</span>
									</div>{/if}
								{#if video.uploadedAt}<div>
...
{#if showEditModal && editingVideo}
	...
	>
		<div
			...
			role="document"
		>
			<h2 style="font-size:15px; font-weight:500; color:#c8d870; margin-bottom:20px;">
				Edit Video
			</h2>
